...
export function createTypographyVortexRenderer(host, canvas, getOptions) {
  ...
  const spawnPointerDust = (time, radius) => {
    const options = getOptions();
  ...
  const spawnAmbientDust = (time, ambientWidth) => {
    const options = getOptions();
  ...
  const updateDust = (time, deltaTime) => {
    const options = getOptions();
