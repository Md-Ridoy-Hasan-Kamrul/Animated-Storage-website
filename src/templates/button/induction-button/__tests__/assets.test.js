...
import {
  ...
  PREVIEW_STILL,
  PREVIEW_STILL_DISK,
  PUBLIC_INDUCTION_HTML_SHA256,
  VALENCE_CORE_HTML_SHA256,
  VARIANT_ID,
} from '../constants';

const ROOT = path.resolve(__dirname, '../../../../../');

describe('Induction packaged assets', () => {
  ...
  it('keeps verified Neuform + valence-core provenance', () => {
    const neuform = fs.readFileSync(
      path.join(ROOT, 'src/templates/button/induction-button/sources/NeuformIsolatedEffects.tsx.txt'),
      'utf8',
    );
    const html = fs.readFileSync(
      path.join(ROOT, 'src/templates/button/induction-button/sources/valence-core.html'),
      'utf8',
    );
    ...
    expect(crypto.createHash('sha256').update(html).digest('hex')).toBe(VALENCE_CORE_HTML_SHA256);

    expect(neuform).toContain('export const InductionButton');
    expect(neuform).toContain('buildFocusedDocument');
    expect(neuform).toContain('#bg-canvas');
    expect(neuform).toContain('#btn');
    expect(neuform).toContain('valence-core');
    expect(html).toContain('id="bg-canvas"');
    expect(html).toContain('id="btn"');
    expect(html).toContain('VALENCE CORE');
    expect(html).toContain('u_arcs');
    expect(html).toContain('webgl');
  });

  it('publishes focused public document with threeui focus isolation', () => {
    const focused = fs.readFileSync(path.join(ROOT, 'public/effects/induction-button.html'), 'utf8');
    const host = fs.readFileSync(
      path.join(ROOT, 'src/templates/button/induction-button/ShaderButtons.jsx'),
      'utf8',
    );
    ...
    expect(focused).toContain('#050505');
    expect(focused).toContain('VALENCE CORE');
