...
import {
  ...
  PREVIEW_STILL,
  PREVIEW_STILL_DISK,
  PUBLIC_GLASSMORPHISM_CTA_HTML_SHA256,
} from '../constants';

const ROOT = path.resolve(__dirname, '../../../../../');

describe('Glassmorphism CTA Rectangle Buttons packaged assets', () => {
  ...
  it('keeps verified NeuformIsolatedEffects and glassmorphism-cta HTML provenance', () => {
    const component = fs.readFileSync(
      path.join(
        ROOT,
        'src/templates/button/glassmorphism-cta-rectangle-buttons/sources/NeuformIsolatedEffects.tsx.txt',
      ),
      'utf8',
    );
    const html = fs.readFileSync(
      path.join(
        ROOT,
        'src/templates/button/glassmorphism-cta-rectangle-buttons/sources/glassmorphism-cta.html',
      ),
      'utf8',
    );
    ...
    expect(component).toContain('export const GlassmorphismCta');
    expect(component).toContain('buildFocusedDocument');
    expect(component).toContain('glassmorphism-cta.html');
    expect(component).toContain('.component-wrapper a');
    expect(component).toContain('sandbox="allow-scripts"');
    expect(html).toContain('Generate My Site');
    expect(html).toContain('borderBeamRotation');
    expect(html).toContain('Advisor headshot');
    expect(html).toContain('conic-gradient');
  });

  it('publishes focused public document with threeui focus isolation', () => {
    const focused = fs.readFileSync(
      path.join(ROOT, 'public/effects/glassmorphism-cta.html'),
      'utf8',
    );
    ...
    expect(focused).toContain('Generate My Site');
    expect(focused).toContain('data-threeui-role');
    expect(focused).toContain('data-threeui-preserve-transform');
    expect(focused).toContain('.component-wrapper a');
    expect(focused).toContain('borderBeamRotation');
  });
