...
describe('Liquid Orb Liquid Metal Button packaged assets', () => {
  ...
  it('keeps verified liquid-metal-button.html provenance for liquid orb', () => {
    const html = fs.readFileSync(
      path.join(
        ROOT,
        'src/templates/button/liquid-orb-liquid-metal-button/sources/liquid-metal-button.html',
      ),
      'utf8',
    );
  ...
  it('ships circle-adapted public effect with orb CSS and bridge', () => {
    const publicHtml = fs.readFileSync(
      path.join(ROOT, 'public/effects/liquid-orb-liquid-metal-button.html'),
      'utf8',
    );
    const host = fs.readFileSync(
      path.join(
        ROOT,
        'src/templates/button/liquid-orb-liquid-metal-button/LiquidMetalButton.jsx',
      ),
      'utf8',
    );

    expect(publicHtml).toContain('data-shape="circle"');
    expect(publicHtml).toContain('liquid-metal-circle-variant');
    expect(publicHtml).toContain('clamp(56px, 10vmin, 72px)');
    expect(publicHtml).toContain('aria-label="Add"');
