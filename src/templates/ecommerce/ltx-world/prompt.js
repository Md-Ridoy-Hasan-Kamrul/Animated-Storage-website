/**
 * Exact LTX World Model design prompt — used for “Copy prompt”.
 * Do not alter characters.
 */
export const LTX_WORLD_PROMPT = `Build a single, self-contained \`index.html\` — no build step, no framework, no npm, no
...
═══════════════════════════════════════════════════════════════════════
 ...
 3. Deferring the rear-track fade with setTimeout races an early failure that already
    reopened the bar, leaving the track invisible → apply it in the SAME style recalc
    as the collapse class. Never \`display:none\`, never abruptly drop blur/border.
