/**
 * Exact design prompt for Portfolio Cosmic (copy-as-is).
 * Do not alter characters — used for “Copy prompt” on the detail page.
 */
export const PORTFOLIO_COSMIC_PROMPT = `Prompt to recreate this landing page:
...
### Smooth scroll navigation (required)
- Navbar links Home → #home, Work → #work, Resume → #resume.
- On click: preventDefault, animate window scroll with ease-out cubic (~900ms) to the section id.
- Active nav pill updates from scroll position (section nearest viewport).
- Logo mark and “See Works” CTA also use the same smooth scroll helper.
- Do not rely only on CSS scroll-behavior for nav — use an explicit animated scroll.

### Media assets
- Journal + Explorations thumbnails must use working Unsplash (or equivalent CDN) URLs that return HTTP 200.
- Replace any 404 image ids; every card must show a real photograph (no broken-image icon).`;

export default PORTFOLIO_COSMIC_PROMPT;
