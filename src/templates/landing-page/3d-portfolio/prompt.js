export const JACK_3D_PROMPT = `Build a 3D Creator portfolio landing page for "Kamrul" using React, TypeScript, Tailwind CSS, Framer Motion, and Lucide React. The page has a dark theme (#0C0C0C background) with the font Kanit (Google Fonts, weights 300-900). The page title is "Kamrul -- 3D Creator".
...
NAV SMOOTH SCROLL (required): Clicking About / Price / Projects / Contact must animated-scroll to section ids #about, #price, #projects, #contact. Use Framer Motion animate() from current window.scrollY to the target section's offsetTop with duration ~1.05s and easing [0.25, 0.1, 0.25, 1]. preventDefault on anchor clicks. Contact Me button also scrolls to #contact. Add a Contact section at the end with id="contact".

Hero Heading: Massive h1 with text "Hi, i'm Kamrul" (lowercase "i", curly apostrophe via &apos;). Uses the .hero-heading gradient text class. Font-black, uppercase, tracking-tight, leading-none, whitespace-nowrap. MUST fill the full viewport/container width with no side gap — use a fit-to-width scaler (measure text scrollWidth vs container clientWidth and set font-size so text spans 100% width). Margin top: mt-6 sm:mt-4 md:-mt-5. Wrapped in overflow-hidden container.
...
3 sticky-stacking project cards. CRITICAL STACK BEHAVIOR: each card wrapper is position sticky; top: 0; height: 100dvh; higher z-index for later cards. While scrolling, the current card STAYS FIXED in place and the NEXT card slides UP OVER it (one on top of another). Previous cards slightly scale down (targetScale = 1 - (totalCards - 1 - index) * 0.05) with transform-origin top center. Offset each card visually with marginTop: index * 28px so a peek of the previous card remains visible. Do NOT put sticky inside a short non-sticky parent that unpins early — the sticky element itself must be the full-viewport layer.

Scale calculation: targetScale = 1 - (totalCards - 1 - index) * 0.05. Each card offset by top/margin: \${index * 28}px.
...
Col2 image: https://images.higgs.ai/?default=1&output=webp&url=https%3A%2F%2Fd8j0ntlcm91z4.cloudfront.net%2Fuser_38xzZboKViGWJOttwIXH07lWA1P%2Fhf_20260412_055818_9d062121-ad7e-46b9-999a-1a6a802ef1ee.png&w=1280&q=85
REUSABLE COMPONENTS
