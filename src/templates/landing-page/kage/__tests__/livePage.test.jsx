...
import { KAGE_DEFAULT_PROPS, SOURCE_URL } from '../constants';
...
describe('KageLandingPage host', () => {
  it('loads the byte-exact local document URL', () => {
    render(
      <KageLandingPage {...KAGE_DEFAULT_PROPS} />,
    );
    const frame = screen.getByTestId('kage-frame');
    expect(frame).toHaveAttribute('data-source', SOURCE_URL);
    expect(frame).toHaveAttribute('data-title', 'Kage');
  ...
  it('peels typography props for the frame', () => {
    ...
    expect(type.headingFont).toBe('onest');
    expect(type.primaryColor).toBe('#e0231c');
    expect(rest.className).toBe('extra');
    expect(rest.headingFont).toBeUndefined();
  });
});

describe('KAGE LivePage', () => {
  it('mounts the shader frame host without a back control in embed chrome', () => {
    ...
    expect(container.querySelector('.kage-page')).toBeTruthy();
