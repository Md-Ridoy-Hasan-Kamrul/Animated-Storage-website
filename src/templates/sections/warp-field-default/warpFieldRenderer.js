...
export const WARP_FIELD_DEFAULTS = { variant: "streaks", speed: 15, streakOpacity: 0.6, tileOpacity: 0.9, fov: 75, brightness: 1, hue: 0, saturation: 1 };
...
const SPEED_SCALE = { streaks: 1, letters: 0.5, keycaps: 0.7, hyperspace: 2.4 };
const BACKGROUND = { streaks: 0x02040a, letters: 0x02040a, keycaps: 0x03070c, hyperspace: 0x01020a };
...
function createTileLayer(group, opacity) {
  ...
  return {
    ...
    setOpacity(_streakOpacity, tileOpacity) {
      ...
      tiles.forEach((tile) => { (tile.material).opacity = tileOpacity; });
...
function createGlyphAtlas(color) {
  const columns = 6;
  const rows = 6;
  const cell = 128;
...
function createLetterLayer(group, opacity) {
...
function createSparkLayer(group, count, size, color, opacity, radiusSpread) {
  const geometry = new THREE.BufferGeometry();
...
function createKeycapLayer(scene, group, opacity) {
  const atlas = createGlyphAtlas("#ffffff");
...
function createHyperspaceLayer(group, opacity) {
...
export function createWarpFieldRenderer(canvas, getOptions) {
  ...
  const variant = WARP_FIELD_VARIANTS.includes(startOptions.variant) ? startOptions.variant : WARP_FIELD_DEFAULTS.variant;
  ...
  const layers = [createStreakLayer(group, STREAK_SETTINGS[variant], startOptions.streakOpacity)];
