...
describe('ThreeDPaper Certificate host', () => {
  it('loads the Certificate document URL', () => {
  ...
  it('exposes certificate variant on the host', () => {
    const { container } = render(<ThreeDPaper {...THREE_D_PAPER_DEFAULT_PROPS} />);
    const host = container.querySelector('.three-d-paper');
    expect(host).toHaveAttribute('data-variant', 'certificate');
  });

  it('falls back invalid variants to certificate', () => {
...
describe('Certificate 3D Paper LivePage', () => {
  it('mounts the shader frame host without a back control in embed chrome', () => {
    ...
    expect(container.querySelector('.certificate-3d-paper-page')).toBeTruthy();
