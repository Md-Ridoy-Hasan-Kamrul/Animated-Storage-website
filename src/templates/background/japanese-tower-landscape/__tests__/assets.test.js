...
describe('Japanese Tower Landscape packaged assets', () => {
  ...
  it('serves the Japan Towers document from the effects URL', () => {
    ...
    expect(html).toContain("id:'JAPAN'");
    expect(html).toContain('applyStyle(0)');
    expect(html).toContain('window.__style');
    expect(html).toContain('data-threeui-country');
    expect(html).toContain('TENSHU GROWTH STUDY');
  });
  ...
  it('keeps provenance Towers.html identical to the registered source', () => {
    ...
    expect(provenance).toContain("id:'JAPAN'");
    expect(provenance).toContain('buildTenshu');
