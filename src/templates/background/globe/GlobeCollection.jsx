...
const EnergyOrbVariant = lazy(() => import("./EnergyOrb").then(module => ({
  default: module.EnergyOrb
})));
function clamp(value, minimum, maximum) {
  return Math.min(maximum, Math.max(minimum, value));
}
function buildGlobeDocument(variant) {
...
(function () {
  ...
  const adaptedSource = SOURCE_BY_VARIANT[variant].replaceAll("performance.now()", "window.__GLOBE_COLLECTION_NOW()");
  return adaptedSource.replace(/<script[^>]+cloudflareinsights\.com[^>]*><\/script>/gi, "").replace("</head>", `${focusStyles}${controls}</head>`);
}
function GlobeCanvasScene({
  ...
  return <div className={`threeui-background globe-collection globe-collection--${variant}${className ? ` ${className}` : ""}`} style={{
    background: isTangled ? "#131313" : "#272727",
    pointerEvents: "auto",
    ...style
  }}>
      <iframe ref={iframeRef} title={isTangled ? "Interactive tangled constellations globe" : "Interactive network globe"} srcDoc={source} sandbox="allow-scripts" onLoad={postControls} style={{
      position: "absolute",
      inset: 0,
      display: "block",
      width: "100%",
...
export function GlobeCollection(props) {
  ...
  return <Suspense fallback={<div className="threeui-background energy-orb" style={{
    background: "#05030e"
  }} />}>
      <EnergyOrbVariant {...energyOrbProps} />
    </Suspense>;
}
export function TangledConstellations(props) {
  return <GlobeCanvasScene {...props} variant="tangled-constellations" />;
}
export function NetworkGlobe(props) {
