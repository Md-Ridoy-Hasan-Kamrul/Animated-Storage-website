...
function sourceForVariant(sourceUrl, variant) {
...
export function SparkBadge({
  ...
  sourceUrl = "/effects/spark-badge.html",
  ...
  return <div ref={hostRef} className={`spark-badge${className ? ` ${className}` : ""}`} data-state={!mounted ? "paused" : ready ? "ready" : "loading"} data-variant={variant}>
      {mounted ? <iframe ref={iframeRef} className={`spark-badge__frame${ready ? " is-ready" : ""}`} title={VARIANT_TITLES[variant]} src={frameSource} sandbox="allow-scripts" loading="eager" onLoad={() => {
      postControls();
      setReady(true);
    }} /> : null}
    </div>;
