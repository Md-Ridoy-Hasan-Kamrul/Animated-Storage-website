...
import { GATEWAY_FLOW_DEFAULT_PROPS, SOURCE_URL, VARIANT_ID } from '../constants';
...
describe('Gateway Flow ConstellationField host', () => {
  it('mounts the gateway-flow iframe with the CSP-safe public scene URL', () => {
    const { container } = render(
      <ConstellationField {...GATEWAY_FLOW_DEFAULT_PROPS} sourceUrl={SOURCE_URL} />,
    );
    ...
    expect(frame.getAttribute('title')).toBe('Gateway Flow');
  });
...
describe('Gateway Flow Constellation Field LivePage', () => {
  it('mounts the shader frame host without a back control in embed chrome', () => {
    ...
    expect(container.querySelector('.gateway-flow-constellation-field-page')).toBeTruthy();
