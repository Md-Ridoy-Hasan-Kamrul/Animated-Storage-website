...
const paintCinematic = (context, width, height, time) => {
  ...
  const cx = width / 2,
    cy = top + frameHeight / 2,
    radius = frameHeight * 0.325;
  const phase = (time % CINEMATIC_CYCLE + CINEMATIC_CYCLE) % CINEMATIC_CYCLE,
    counting = phase < 7;
  ...
  const pitch = frameHeight / 9,
    offset = time * pitch * 2.4 % pitch,
    perfWidth = width * 0.011,
    perfHeight = pitch * 0.34;
  ...
  for (const corner of ["tl", "tr", "bl", "br"]) {
    const x = corner.endsWith("l") ? width * 0.085 : width * 0.915,
      y = corner.startsWith("t") ? top + frameHeight * 0.16 : bottom - frameHeight * 0.16;
  ...
  if (counting) {
    ...
    for (let tick = 0; tick < 12; tick += 1) {
      const angle = start + tick / 12 * Math.PI * 2,
        inner = tick % 3 === 0 ? radius * 1.055 : radius * 1.028;
      context.beginPath();
...
const RUNNER_HEAD = ["....111111....", "...13333331...", "..1333333331..", "..1355555531..", "..1355555531..", "..1333333331..", "...13333331...", "....111111....", "...12222221...", ".122222222221.", ".124444444421.", ".122222222221.", "..1222222221.."];
const RUNNER_LEGS = {
...
function drawGem(context, x, y, spin) {
  const gemWidth = [8, 6, 2, 6][spin],
    half = gemWidth / 2,
    inset = gemWidth > 3 ? 1 : 0;
  context.fillStyle = "#20140c";
...
const paintNintendo = (context, width, height, time) => {
  const groundY = GROUND_ROW,
    phase = (time % NINTENDO_CYCLE + NINTENDO_CYCLE) % NINTENDO_CYCLE;
  ...
  const spin = Math.floor(time * 8) % 4,
    bob = [0, 1, 1, 0][Math.floor(time * 6) % 4];
  ...
  const title = "RASTER RUN",
    titleWidth = pixelTextWidth(title, 3),
    titleX = Math.round((width - titleWidth) / 2);
  ...
  context.restore();
  const credit = "(C) 1987 THREEUI",
    creditX = Math.round((width - pixelTextWidth(credit, 1)) / 2);
  ...
  if (Math.floor(time * 1.6) % 2 === 0) {
    const start = "PUSH START",
      startX = Math.round((width - pixelTextWidth(start, 2)) / 2);
