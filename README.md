...
- **No duplicate template prompts (one prompt → one card)** — example: `src/templates/landing-page/3d-portfolio/prompt.js` already built **3D Portfolio**. If the user pastes that same prompt again for a new card, **stop and refuse**. Do not build a second animation/card from a prompt that already has a `prompt.js`. Reuse the existing template or require a different prompt.
