/* Build CSP-safe focused Floating Dots CTA (NeuformIsolatedEffects) document. */
const fs = require('fs');
const path = require('path');
const crypto = require('crypto');

const ROOT = path.resolve(__dirname, '..');
const SRC = path.join(ROOT, 'src/templates/button/floating-dots-cta-rectangle-buttons/sources');
const OUT = path.join(ROOT, 'public/effects/floating-dots-cta.html');

const provenance = fs.readFileSync(path.join(SRC, 'floating-dots-cta.html'), 'utf8');
...
if (!focused.includes('Sign Up')) throw new Error('floating dots label missing');
if (!focused.includes('floating-points')) throw new Error('floating-points animation missing');
if (!focused.includes('points_wrapper')) throw new Error('points_wrapper markup missing');
...
console.log('floating-dots-cta.html bytes', bytes);
console.log('floating-dots-cta.html sha256', sha);
