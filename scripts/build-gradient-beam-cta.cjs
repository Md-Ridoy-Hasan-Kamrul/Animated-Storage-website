/* Build CSP-safe focused Gradient Beam CTA (NeuformIsolatedEffects) document. */
const fs = require('fs');
const path = require('path');
const crypto = require('crypto');

const ROOT = path.resolve(__dirname, '..');
const SRC = path.join(ROOT, 'src/templates/button/gradient-beam-cta-rectangle-buttons/sources');
const OUT = path.join(ROOT, 'public/effects/gradient-beam-cta.html');

const provenance = fs.readFileSync(path.join(SRC, 'gradient-beam-cta.html'), 'utf8');
...
if (!focused.includes('Start Building')) throw new Error('gradient beam label missing');
if (!focused.includes('beam-spin')) throw new Error('beam-spin animation missing');
...
console.log('gradient-beam-cta.html bytes', bytes);
console.log('gradient-beam-cta.html sha256', sha);
