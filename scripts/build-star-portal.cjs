/* Build CSP-safe focused Star Portal (NeuformIsolatedEffects starfield) document. */
const fs = require('fs');
const path = require('path');
const crypto = require('crypto');

const ROOT = path.resolve(__dirname, '..');
const SRC = path.join(ROOT, 'src/templates/button/star-portal/sources');
const OUT = path.join(ROOT, 'public/effects/star-portal.html');

const provenance = fs.readFileSync(path.join(SRC, 'imaginie-starfield.html'), 'utf8');

const mode = 'dark';
const background = '#0d0a12';
const invertBackground = false; // nativeMode dark === mode dark
...
if (!focused.includes('holo-btn')) throw new Error('holo button missing');
if (!focused.includes('ambient-starfield')) throw new Error('ambient starfield missing');
if (!focused.includes('portal-stars')) throw new Error('portal stars missing');
if (!focused.includes('Begin the journey')) throw new Error('holo label missing');
...
console.log(JSON.stringify({ path: 'public/effects/star-portal.html', bytes, sha256: sha }, null, 2));
