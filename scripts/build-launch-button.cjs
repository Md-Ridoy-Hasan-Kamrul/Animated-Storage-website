/* Build CSP-safe focused Launch Button (NeuformIsolatedEffects) document. */
const fs = require('fs');
const path = require('path');
const crypto = require('crypto');

const ROOT = path.resolve(__dirname, '..');
const SRC = path.join(ROOT, 'src/templates/button/launch-button-rectangle-buttons/sources');
const OUT = path.join(ROOT, 'public/effects/launch-button.html');

const provenance = fs.readFileSync(path.join(SRC, 'launch-button.html'), 'utf8');
...
if (!focused.includes('Initialize Launch')) throw new Error('launch label missing');
if (!focused.includes('group/btn')) throw new Error('button markup missing');
...
console.log('launch-button.html bytes', bytes);
console.log('launch-button.html sha256', sha);
