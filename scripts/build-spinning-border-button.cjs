/* Build CSP-safe focused Spinning Border Button (NeuformIsolatedEffects) document. */
const fs = require('fs');
const path = require('path');
const crypto = require('crypto');

const ROOT = path.resolve(__dirname, '..');
const SRC = path.join(ROOT, 'src/templates/button/spinning-border-button-rectangle-buttons/sources');
const OUT = path.join(ROOT, 'public/effects/spinning-border-button.html');

const provenance = fs.readFileSync(path.join(SRC, 'spinning-border-button.html'), 'utf8');
...
if (!focused.includes('Request Demo')) throw new Error('spinning border label missing');
if (!focused.includes('conic-gradient')) throw new Error('conic-gradient missing');
if (!focused.includes('animate-[spin_3s_linear_infinite]')) throw new Error('spin animation missing');
...
console.log('spinning-border-button.html bytes', bytes);
console.log('spinning-border-button.html sha256', sha);
