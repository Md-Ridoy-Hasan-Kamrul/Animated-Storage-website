...
if (canonicalSha !== expectedCanonical) {
  throw new Error(`Canonical Override Grid hash mismatch: ${canonicalSha}`);
}

/** Configured usage: size=48, gap=2, mode=dark, speed=1 */
const BLOCK_SIZE = 48;
const BLOCK_GAP = 2;
const BACKGROUND = '#050505';

function patchOverrideGrid(source) {
  return source
    .replace('const blockSize = 48;', `const blockSize = ${BLOCK_SIZE};`)
    .replace('const blockGap = 2;', `const blockGap = ${BLOCK_GAP};`)
    .replace(
      'time += 0.04;',
...
const targetJson = JSON.stringify([
  { selector: '#grid-canvas', role: 'background' },
]).replace(/</g, '\\u003c');
...
const patched = patchOverrideGrid(canonical);
const focused = patched
  .replace(/<head([^>]*)>/i, `<head$1>${controlScript}${focusStyle}`)
  .replace(/<\/body>/i, `${focusScript}</body>`);

const outPath = path.join(root, 'public/effects/override-grid-predictive-arc.html');
  ...
  JSON.stringify(
    {
      ...
      hasGrid: focused.includes('id="grid-canvas"'),
      blockSize: focused.includes(`const blockSize = ${BLOCK_SIZE};`),
    },
    null,
    2,
  ),
