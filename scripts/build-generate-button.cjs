/* Build CSP-safe focused Generate Button (NeuformIsolatedEffects) document. */
const fs = require('fs');
const path = require('path');
const crypto = require('crypto');

const ROOT = path.resolve(__dirname, '..');
const SRC = path.join(ROOT, 'src/templates/button/generate-button-rectangle-buttons/sources');
const OUT = path.join(ROOT, 'public/effects/generate-button.html');

const provenance = fs.readFileSync(path.join(SRC, 'generate-button.html'), 'utf8');
...
if (!focused.includes('Generate')) throw new Error('generate label missing');
if (!focused.includes('letter-anim')) throw new Error('letter-anim missing');
if (!focused.includes('flicker')) throw new Error('flicker missing');
if (!focused.includes('preserveTransform')) throw new Error('preserveTransform missing');
if (!focused.includes('.btn-wrapper')) throw new Error('btn-wrapper selector missing');
...
console.log('generate-button.html bytes', bytes);
console.log('generate-button.html sha256', sha);
