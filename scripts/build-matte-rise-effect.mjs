...
const variant = GALLERY_HEADING_VARIANTS['rising-diagonal'];
...
<script src="/effects/matte-rise.js"></script>
</body>
</html>
`;
...
fs.writeFileSync(path.join(outDir, 'matte-rise.js'), scriptBody);
fs.writeFileSync(path.join(outDir, 'matte-rise.html'), html);
...
console.log(
  JSON.stringify(
    {
      js: { bytes: size('public/effects/matte-rise.js'), sha256: sha('public/effects/matte-rise.js') },
      html: {
        bytes: size('public/effects/matte-rise.html'),
        sha256: sha('public/effects/matte-rise.html'),
      },
      checks: {
        twelveWorks: scriptBody.includes('TWELVE WORKS'),
        matte: scriptBody.includes("FIELD = 'matte'"),
        spring: scriptBody.includes('SPRING = 1'),
