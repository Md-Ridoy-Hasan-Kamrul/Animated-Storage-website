...
const srcPath = path.join(root, 'scripts/nebula-source.json');
const data = JSON.parse(fs.readFileSync(srcPath, 'utf8'));
const outDir = path.join(root, 'src/templates/background/nebula-structure-flow/sources');
...
const htmlFile = data.files.find((f) => f.path.endsWith('julian-vance-nebula.html'));
const cssFile = data.files.find((f) => f.path.endsWith('threeui.css'));
const craftFile = data.files.find((f) => f.path.endsWith('NeuformCraftEffects.tsx'));
...
fs.writeFileSync(path.join(outDir, 'julian-vance-nebula.html'), htmlFile.code);
fs.writeFileSync(path.join(outDir, 'threeui.css'), cssFile.code);
fs.writeFileSync(path.join(outDir, 'NeuformCraftEffects.tsx'), craftFile.code);

const background = '#09090b';
const targets = [{ selector: '#bg-canvas', role: 'background' }];
...
const focusedPath = path.join(effectsDir, 'nebula-structure-flow.html');
fs.writeFileSync(focusedPath, focused);

const htmlBuf = fs.readFileSync(path.join(outDir, 'julian-vance-nebula.html'));
...
console.log(
  JSON.stringify(
    {
      julianBytes: htmlBuf.length,
      julianSha: crypto.createHash('sha256').update(htmlBuf).digest('hex'),
      ...
      hasBgCanvas: focused.includes('bg-canvas'),
      hasThree: focused.includes('three.js/r128'),
