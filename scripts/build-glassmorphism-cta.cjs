/* Build CSP-safe focused Glassmorphism CTA (NeuformIsolatedEffects) document. */
const fs = require('fs');
const path = require('path');
const crypto = require('crypto');

const ROOT = path.resolve(__dirname, '..');
const SRC = path.join(ROOT, 'src/templates/button/glassmorphism-cta-rectangle-buttons/sources');
const OUT = path.join(ROOT, 'public/effects/glassmorphism-cta.html');

const provenance = fs.readFileSync(path.join(SRC, 'glassmorphism-cta.html'), 'utf8');

const mode = 'dark';
const background = '#111318';
const targetJson = JSON.stringify([
  { selector: '.component-wrapper a', role: 'button', preserveTransform: true },
]).replace(/</g, '\\u003c');
...
if (!focused.includes('Generate My Site')) throw new Error('glassmorphism label missing');
if (!focused.includes('borderBeamRotation')) throw new Error('borderBeamRotation missing');
if (!focused.includes('Advisor headshot')) throw new Error('advisor portrait missing');
if (!focused.includes('preserveTransform')) throw new Error('preserveTransform missing');
if (!focused.includes('.component-wrapper a')) throw new Error('anchor selector missing');
...
console.log('glassmorphism-cta.html bytes', bytes);
console.log('glassmorphism-cta.html sha256', sha);
