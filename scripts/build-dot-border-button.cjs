/* Build CSP-safe focused Dot Border Button (NeuformIsolatedEffects) document. */
const fs = require('fs');
const path = require('path');
const crypto = require('crypto');

const ROOT = path.resolve(__dirname, '..');
const SRC = path.join(ROOT, 'src/templates/button/dot-border-button-rectangle-buttons/sources');
const OUT = path.join(ROOT, 'public/effects/dot-border-button.html');

const provenance = fs.readFileSync(path.join(SRC, 'dot-border-button.html'), 'utf8');
...
if (!focused.includes('Start Creating')) throw new Error('dot border label missing');
if (!focused.includes('btn-wrapper')) throw new Error('btn-wrapper markup missing');
...
console.log('dot-border-button.html bytes', bytes);
console.log('dot-border-button.html sha256', sha);
