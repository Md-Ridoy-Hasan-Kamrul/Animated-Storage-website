...
const srcPath = path.join(root, 'scripts/japanese-tower-source.json');
const data = JSON.parse(fs.readFileSync(srcPath, 'utf8'));

const outDir = path.join(root, 'src/templates/background/japanese-tower-landscape/sources');
...
if (!htmlFile?.code.includes('<!doctype html>') && !htmlFile?.code.includes('<!DOCTYPE html>')) {
...
/** Maps JapaneseTowerLandscape `country` query to STYLES[] index (authored order). */
const countryBoot = `<script data-threeui-country>
(function () {
...
focused = focused.replace(/<\/body>/i, `${countryBoot}</body>`);
  ...
  JSON.stringify(
    {
      ...
      hasApplyStyle0: focused.includes('applyStyle(0)'),
