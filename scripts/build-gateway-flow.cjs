/* Build CSP-safe focused Gateway Flow document (dark defaults). */
const fs = require('fs');
...
const SRC = path.join(
  ROOT,
  'src/templates/background/gateway-flow-constellation-field/sources',
);
const OUT = path.join(ROOT, 'public/effects/gateway-flow.html');

const provenance = fs.readFileSync(path.join(SRC, 'gateway-flow.html'), 'utf8');
...
const targetJson = JSON.stringify([
  { selector: '#flow-canvas', role: 'background' },
]).replace(/</g, '\\u003c');
...
if (!focused.includes('flow-canvas')) throw new Error('canvas missing');
...
console.log('gateway-flow.html bytes', bytes);
console.log('gateway-flow.html sha256', sha);
