/* Build CSP-safe focused Gradient CTA (NeuformIsolatedEffects) document. */
const fs = require('fs');
const path = require('path');
const crypto = require('crypto');

const ROOT = path.resolve(__dirname, '..');
const SRC = path.join(ROOT, 'src/templates/button/gradient-cta-rectangle-buttons/sources');
const OUT = path.join(ROOT, 'public/effects/gradient-cta.html');

const provenance = fs.readFileSync(path.join(SRC, 'gradient-cta.html'), 'utf8');
...
if (!focused.includes('Start Free Pilot')) throw new Error('gradient cta label missing');
if (!focused.includes('#FFEBB1')) throw new Error('amber gradient missing');
if (!focused.includes('translate-y-full')) throw new Error('hover sheen missing');
...
console.log('gradient-cta.html bytes', bytes);
console.log('gradient-cta.html sha256', sha);
